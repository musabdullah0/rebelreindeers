...
class Darknet(nn.Module):
    ...
    def forward(self, x, targets=None, rudi=False):
        # batch_size, c, h, w
        img_size = x.size(2)
        ind = -2
        self.loss = None
        ...
        if rudi:
            rudi_outputs = to_cpu(torch.cat(rudi_outputs, 1))
            return yolo_outputs, rudi_outputs if targets is None else (loss, yolo_outputs, rudi_outputs)
