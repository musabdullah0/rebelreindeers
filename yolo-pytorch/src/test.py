...
from data_process.kitti_dataloader import create_test_dataloader, create_anotha_dataloader
...
if __name__ == '__main__':
    ...
    with torch.no_grad():
        for batch_idx, (img_paths, imgs_bev) in enumerate(dataloader):
            ...
            # writing csv file with path to image and detections
            print(img_paths[0])
            csv_fn = os.path.basename(img_paths[0])[:-4]
            # path_to_csv = os.path.join('./results', '{}.csv'.format(csv_fn))
            path_to_csv = './yolo-outputs.csv'
            with open(path_to_csv, 'a', newline='') as file:
                writer = csv.writer(file)

                for i, pred in enumerate(objects_pred):
                    if pred is None:
                        continue
                    kitti_format = pred.to_csv_format()
                    # print(kitti_format)
                    writer.writerow([img_paths[0], i, kitti_format])
